...
if ($pos !== false){
	$variables = substr($_SERVER['REQUEST_URI'],$pos+1,strlen($_SERVER['REQUEST_URI']));
...
$moduleRecord = getRecords("Module",false,"page_name='".$page_name."'");
if (is_array($moduleRecord)){
	$id = $moduleRecord[0]["id"];
...
<?php
	if (!isset($_REQUEST["id_content"])){
		...
		$breadcrumb	.= '&nbsp;&nbsp;&nbsp;<a href="'.$page_name.'">Index</a>&nbsp;&nbsp;';
