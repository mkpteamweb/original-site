 ...
 *
 *   $Id: index.php, v1.20 2003/05/30 23:32:50 bugada Exp $
 *
 *
 ***************************************************************************/
...
// Added by MrScripto for File Moving
function copy_file($filename, $newdest)
{
	if (file_exists($filename))
		copy($filename, $newdest);

	if (file_exists("$filename.desc"))
		copy($filename.'.desc', $newdest.'.desc');

	if (file_exists("$filename.dlcnt"))
		copy($filename.'.dlcnt', $newdest.'.dlcnt');
}
...
function scan_dir_for_digest($current_dir, &$message)
{
	...
	if (is_array($liste))
	{
		while (list($filename, $mime) = each($liste))
		{
			...
			// Se il file non Ë validato non lo mostro
...
//
// Controlla quando Ë stato fatto l'ultima manutenzione e, se necessario, ne esegue una nuova
//
function do_maintenance()
{
	...
	// Leggo la data in cui Ë stato fatta l'ultima manutenzione
	...
	// Se Ë passato pi˜ di un giorno allora lancio la manutenzione
...
function contents_dir($current_dir, $directory)
{
  ...
  if(is_array($liste))
  {
    while (list($filename,$mime) = each($liste))
    {
	 ...
	 // Controllo la validazione del file. Se Ë una directory non richiede validazione (per ora).
	 if (is_dir("$current_dir/$filename"))
	 {
...
// Se il file non Ë attivato mostro l'icona per l'attivazione
	...
	if ($filestatus == UNVALIDATED)
	{
		echo "&nbsp;
		      <a href=\"index.${phpExt}?action=validatefile&filename=$filename&directory=$directory&".SID."\">
		        <img src=\"images/validate.gif\" width=\"20\" height=\"20\" border=\"0\" alt=\"$mess[214]\" title=\"$mess[214]\"></a>";
	}
	else
	{
	...
	if (!is_dir("$current_dir/$filename"))
	{
		echo "&nbsp;
			 <a href=\"javascript:delobj('deletefile', '$filename', '$directory');\">
		        <img src=\"images/delete.gif\" width=\"20\" height=\"20\" border=\"0\" alt=\"$mess[169]\" title=\"$mess[169]\"></a>";
    }
    else
    {
...
if ($grants[$user_status][MODALL] || ($upl_user == $logged_user_name && $grants[$user_status][MODOWN]))
{
	if (!is_dir("$current_dir/$filename") || $grants[$user_status][MODALL])
	{
		...
		<img src=\"images/rename.gif\" border=\"0\" alt=\"$mess[191]\" title=\"$mess[191]\"></a>";
    }
}
else
{
...
//
// Added by MrScripto for Moving File Link
//
if ($grants[$user_status][MOVE])
{
	if (!is_dir("$current_dir/$filename"))
	{
		echo "<a href=\"movefile.${phpExt}?action=move&filename=$filename&directory=$directory&".SID."\">
		<img src=\"images/movefile.gif\" border=\"0\" alt=\"$mess[M07]\" title=\"$mess[M07]\"></a>";
    }
}
else
{
...
echo "        <a href=\"javascript:popupmail('$filename', '$directory')\">
             ...
             alt=\"$mess[68]\" title=\"$mess[68]\" width=\"20\" height=\"20\" border=\"0\"></a>";
  }
  else
	echo " <img src=\"images/empty.gif\" width=\"20\" height=\"20\" border=\"0\"> ";
}
...
echo "        <a href=\"index.${phpExt}?action=downloadfile&filename=$filename&directory=$directory&".SID."\">
             ...
             alt=\"$mess[23]\" title=\"$mess[23]\" width=\"20\" height=\"20\" border=\"0\"></a>";
...
//
// Cancella una directory. Se non Ë vuota ne cancella ricorsivamente tutto
...
global $order,$totalsize,$font,$tablecolor,$bordercolor,$headercolor, $logged_user_name;
global $headerfontcolor,$normalfontcolor,$user_status, $grants, $phpExt, $num_of_upload_boxes;
...
if ($directory != '' && !$logged_user_name) {
   $public_dir = 'public_downloads';
   $passed_dir = split("/", $directory);
   	if($passed_dir[0] == $public_dir){
   		$directory = $directory;
   	} else {

   		if (check_is_user_session_active($directory)) {
    			$directory = $directory;
    		} else {
    			$directory = '';
    			$user_status = '5';
...
if ($grants[$user_status][VIEW])
{
  list_dir($directory);
	...
	if ($grants[$user_status][VIEWOWN])
	{
		if (!$logged_user_name)
		{
			$logged_user_name = 'public_downloads';
		}
		if (!$directory)
		{
			$directory = $logged_user_name;
		}
		//else {
		//	$directory = ".$logged_user_name."/".$directory.";
		//}
	list_dir($directory);
	}
}
...
if ($grants[$user_status][UPLOAD])
{
  ...
  //Added for multiple file uploads
  for ($x=1;$x<($num_of_upload_boxes + 1);$x++)
  {
  echo "            <tr>\n";
  echo "              <td align=\"left\" width=\"15%\"><font size=\"1\" face=\"$font\" color=\"$normalfontcolor\">$mess[21] $x</font></td>\n";
  echo "              <td align=\"left\" width=\"90%\" colspan=\"2\">\n";
  echo "                <input type=\"file\" class=\"vform\" name=\"userfile$x\" size=\"50\" />\n";
  echo "              </td>\n";
  echo "            </tr>\n";
  echo "            <tr> \n";
  echo "              <td align=\"left\" width=\"15%\"><font size=\"1\" face=\"$font\" color=\"$normalfontcolor\">$mess[22] $x</font></td>\n";
  echo "              <td align=\"left\" width=\"70%\">\n";
  echo "                <input type=\"text\" name=\"description$x\" class=\"vform\" size=62>\n";
  echo "              </td>\n";
  echo "            </tr>\n";
  }
  echo "            <tr>\n";
...
// Se l'utente Ë abilitato mostro il comando di copia via http
...
// Se l'utente Ë abilitato mostro il comando creazione nuova directory remota
...
// Controllo se Ë ora di fare manutenzione
do_maintenance();
...
switch($action)
{
	case 'deletefile';
		...
		if (is_path_safe($directory, $filename))
	    {
			...
			{
				// Cancello solo se l'utente Ë amministratore/superutente o il proprietario del file
				if ($grants[$user_status][DELALL] || ($upl_user == $logged_user_name && $grants[$user_status][DELOWN]))
				{
	...
	case 'validatefile';
		...
		// Valido solo se l'utente Ë abilitato a farlo
	...
	case 'deletedir';
		...
		if (is_path_safe($directory, $filename))
	    {
			// Cancello solo se l'utente Ë abilitato
			if ($grants[$user_status][DELALL])
			{
	...
	case 'createdir';
		...
		if (is_path_safe($directory, $filename))
		{
			...
			if ($filename != '')
			{
				// Creo solo se l'utente Ë abilitato
				if ($grants[$user_status][MKDIR])
				{
	...
	case 'view';
		...
		if (!$grants[$user_status][VIEWOWN] && $user_status != ADMIN)
		{
...
{
		$message = $mess[40];
		$userfile = $_FILES['userfile'.$x];
	      	$description = $_POST['description'.$x];
		$userfile_name = strtolower($userfile['name']);
		...
		// Non permetto l'upload di alcuni tipi di files nË al di fuori del ramo di upload
...
//END FOR STATEMENT and record messages for each file BY MrScripto BELOW
$message2 .= "$x - $message <br>";
   }
   		place_header($message2);
		show_contents();
		break;
	...
	case 'rename';
		...
		if ($grants[$user_status][MODALL] || ($upl_user == $logged_user_name && $grants[$user_status][MODOWN]))
		{
			...
			{
				// Controllo che il nome di destinazione non esista giý
				...
				{
					place_header($mess[198]); // nome file destinazione giý esistente
				}
			}
			else
			{
...
// Added by MrScripto for file move
	case 'movefile';
		// Entro nella directory corretta
		$upload_path = $include_location.$uploads_folder_name;
		$current_dir = $upload_path;
		if ($directory != '')
			$current_dir = "".$upload_path."/".$directory."";

		$new_dest_dir = $new_dir;
		...
		// Check to see if there is already a file named the same in destination directory
		if (file_exists("".$new_dest_dir."".$filename.""))
		{
			place_header($mess[M09]);
			show_Contents();
			break;
		}
		...
		if (is_path_safe($directory, $filename))
			...
			else
			{
				...
				if ($grants[$user_status][MOVE])
				{
					copy_file("$current_dir/$filename", "".$new_dest_dir."".$filename."");
					delete_file("$current_dir/$filename");
					place_header($mess[M11]);
				}
				else
				{
					place_header($mess[M12]);
				}
			}
		}
		else
